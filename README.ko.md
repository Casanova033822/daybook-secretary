...
<p align="center">Windows에서 가볍게 쓰는 일정·할 일 관리 앱입니다.<br>오프라인으로 쓸 수 있고 로그인도 필요 없습니다. 데이터는 내 컴퓨터에만 저장됩니다.</p>
...
![Daybook 메인 화면: 일정, 완료 상태 및 테마 전환](docs/images/daybook-overview.png)

## 설치

1. [다운로드 페이지](https://github.com/Casanova033822/daybook-secretary/releases/latest)에서 **`Daybook-Setup-버전.exe`** 를 받으세요.
2. 설치 파일을 열고 안내대로 진행한 뒤 **日序 (Daybook)** 를 실행하세요.
...
1. **일정 추가:** 이름을 직접 입력하거나 자주 쓰는 프리셋을 고르세요.
2. **시간 정하기:** 시작·종료 시간을 고르고 필요하면 알림이나 반복도 추가하세요. 시간이 미정이면 할 일로 저장할 수 있습니다.
3. **확인하고 완료하기:** 일·주·월 보기에서 일정을 확인하세요. 끝난 일은 왼쪽 상자를 체크하고, 바꾸고 싶을 때는 연필을 누르면 됩니다.
4. **내게 맞게 설정:** 환경설정에서 프리셋, 언어, 알림을 바꿀 수 있습니다. 라이트·다크 모드는 「일정 추가」 위에서 전환하세요.
...
외부 라이브러리와 자료의 라이선스는 [여기](THIRD_PARTY_NOTICES.md)에서 확인할 수 있습니다.
...
Windows x64, Git, Node.js 24.x가 필요합니다.
