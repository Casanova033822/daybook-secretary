...
所有測試都使用獨立產生的假資料，測試截圖、資料庫、錯誤日誌與個別使用者的操作紀錄不會放進公開原始碼
...
打包測試會透過 Chromium CDP 連接測試用 renderer，不會重新開啟已停用的 Node inspector，CDP 也只會在測試命令執行期間啟用，`tests/windows.mjs` 只保留作為舊版整合測試紀錄

乾淨環境驗收要在沒有 `node_modules`、`dist`、`test-results` 的匯出副本中重新安裝、測試、建置與打包，通過只代表來源不依賴原工作目錄的產物，不代表已測過所有 Windows 硬體與企業政策

## 仍需人工確認

- 從 Release 下載並安裝，在沒有 Node.js 與 Noto Sans TC 的電腦確認能啟動而且文字清楚
- 完整、迷你、縮小及收進系統匣時，確認真正的排程小卡會出現而且不搶鍵盤焦點
- 用實體喇叭確認提示音真的有播放，播放完成回呼只能證明軟體流程結束
- 登出再登入 Windows，確認背景自動啟動與關閉此偏好的效果
- 經過真實睡眠、喚醒及時鐘變動後，確認當日漏掉的提醒會合併而且不重複
- 測試多螢幕、不同縮放比例與拔除螢幕後的視窗位置
- 確認覆蓋更新會保留既有資料，完全結束程式後也不會再提醒

首次 GitHub Actions 與另一台實機的驗收要另外留下紀錄，不要把模擬喚醒、同機乾淨副本或聲音回呼當成已完成的人工驗收
