...
1. **予定を追加**：名前を入力するか、瞑想・運動・朝食などのプリセットを選びます。
2. **時間を設定**：開始・終了時刻を設定し、必要に応じて通知や繰り返しを追加して保存します。時間が未定なら「やることとして記録」を選べます。
3. **確認・完了**：日・週・月表示で予定を確認できます。終わったら左のチェックボックスを選択し、変更したいときは鉛筆を押して編集します。
4. **自分好みに調整**：「環境設定」でプリセット、言語、既定の通知を設定できます。ライト／ダークモードは「予定を追加」の上で切り替えられます。
